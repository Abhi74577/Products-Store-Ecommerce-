...
function Product() {
  ...
  return product ? (
    <div className='border-t-1 border-gray-50 pt-10 transition-opacity ease-in duration-500 opacity-100'>
      {/* product data  */} 
      <div className='flex gap-6 sm:gap-8 lg:gap-16 flex-col sm:flex-row sm:justify-between'>

        {/* product image */}
        <div className='flex  flex-col-reverse gap-3 sm:flex-row justify-between lg:w-[45%] '>
          <div className='flex flex-row gap-2 sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-normal'>
            {
              product?.images?.map((item, index) => (
                <img onClick={() => { setImage(item) }} src={item} key={index} className='w-[90px] h-[80px] lg:h-[100px] sm:mb-3 cursor-pointer' alt='' />
              ))
            }
          </div>
        ...
        <div className='flex-1'>
          ...
          <p className='mt-2 sm:mt-5 text-3xl font-medium'>{currency}{product.price}</p>
          <p className='mt-2 sm:mt-5 text-lg font-normal text-gray-700'>{product.description}</p>
          <h1 className='font-medium text-xl mt-5'>Select Size</h1>
          <div className='flex flex-row gap-3 items-center mt-2 sm:mt-5'>
            {
              product.sizes?.map((item, idx) => (
                <bottom className={`py-2 px-4 text-center bg-gray-400 cursor-pointer text-lg ${size == item ? 'border' : ''}`} key={idx} onClick={() => setSize(item)}>{item}</bottom>
              ))
            }
          </div>

          <div className='mt-7'><bottom className='py-3 px-6 text-center bg-black cursor-pointer text-white text-sm  uppercase ' onClick={() => {addToCart(product._id, size)}}>Add To Cart</bottom></div>
